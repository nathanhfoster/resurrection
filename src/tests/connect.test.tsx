...
const setup = (stateContext?: ContextType, dispatchContext?: ContextType, ...args: any[]) => {
...
describe('connect', () => {
  ...
  it('Should mapStateToProps and render them', () => {
    ...
    const wrapper = setup(undefined, undefined, mapStateToProps);
  ...
  it('Should mapDispatchToProps is an object', () => {
    ...
    const wrapper = setup(undefined, undefined, undefined, mapDispatchToProps);
  ...
  it('Should mergeProps', () => {
    ...
    const wrapper = setup(undefined, undefined, mapStateToProps, mapDispatchToProps, mergeProps);
  ...
  it('Should handle a custom options context', () => {
    const mapStateToProps: MapStateToPropsType = ({ key1 }) => ({ key2: key1 });
    const stateContext = React.createContext({ key1: 'key1' });
    const dispatchContext = React.createContext(null);
    const isEqual: EqualityFunctionType = (prevProps, nextProps) => prevProps === nextProps;
    const options = { stateContext, dispatchContext, pure: false, areMergedPropsEqual: isEqual };

    const wrapper = setup(stateContext, dispatchContext, mapStateToProps, undefined, undefined, options);
    const child = wrapper.getByTestId('key2');
    expect(child).toBeDefined();
  });
});
